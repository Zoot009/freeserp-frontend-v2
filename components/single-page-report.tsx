...
// Shared collapsible SEO report body for ONE crawled page. Rendered by both the
// competitor-analysis "SEO Audit" sub-page and the standalone Keyword Analysis
// results page — anywhere we have a single-URL `CrawlData` blob to display.
// Purely presentational: it holds only the expand/collapse UI state.

const DEFAULT_OPEN = ["meta", "content", "keyword", "headings", "links", "images", "technical"]
...
export function SinglePageReport({ crawlData, keyword }: { crawlData: CrawlData; keyword: string }) {
  const [expandedSections, setExpandedSections] = useState<Set<string>>(new Set(DEFAULT_OPEN))
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="border border-border/60 bg-card/30 p-4">
          ...
          <div className="font-mono text-2xl font-bold text-foreground">
            {((crawlData.crawlTime || 0) / 1000).toFixed(1)}s
          </div>
        </div>
      </div>
