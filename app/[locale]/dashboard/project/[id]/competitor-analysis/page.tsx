...
function CompetitorAnalysisContent() {
  ...
  const toggleSerpUrl = (u: string) => {
    const newSelected = new Set(selectedSerpUrls)
    if (newSelected.has(u)) {
      newSelected.delete(u)
    } else {
    ...
    setSelectedSerpUrls(newSelected)
    setError("")
  }
  ...
  const handleAnalyze = async () => {
    setError("")
    const selectedCompetitors = Array.from(selectedSerpUrls)
  ...
  return (
    <div className="page">
        ...
        <div className="row" style={{ flexWrap: "wrap", justifyContent: "flex-end" }}>
          ...
          >
            {isAnalyzing ? (
              ...
              <>
                <Icon.zap />
                {selectedSerpUrls.size > 0
                  ? `Analyze ${selectedSerpUrls.size} competitor${selectedSerpUrls.size === 1 ? "" : "s"}`
                  : "Analyze competitors"}
              </>
            )}
          </button>
        ...
        >
          ...
          {serpCompetitors.length > 0 && (
            <button type="button" className="btn sm" onClick={handleSelectAll}>
              {selectedSerpUrls.size >= 3 ? "Clear selection" : "Select top 3"}
            </button>
          )}
        </div>
        ...
        {loadingSerpData ? (
          ...
          <div className="cmp-list">
            {serpCompetitors.map((c) => {
              ...
              const isSelected = selectedSerpUrls.has(c.url)
              ...
              return (
                <div
                  ...
                  onClick={() => !isOwnSite && toggleSerpUrl(c.url)}
                  role="button"
      ...
      {serpCompetitors.length > 0 && selectedSerpUrls.size > 0 && selectedSerpUrls.size !== 3 && (
        <div
        ...
        >
          ...
          {selectedSerpUrls.size < 3
            ? `Select ${3 - selectedSerpUrls.size} more to continue — exactly 3 required.`
            : `Deselect ${selectedSerpUrls.size - 3} to continue — exactly 3 required.`}
        </div>
      )}
